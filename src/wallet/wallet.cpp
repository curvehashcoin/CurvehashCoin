...
// curvehash: optional setting to unlock wallet for block staking only;
...
//    return n ? n : nTimeReceived;
    ...
    // curvehash: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// curvehash: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // curvehash: timestamp must not exceed spend time
...
void CWallet::AvailableCoinsForStaking(std::vector<COutput> &vCoins, uint32_t nSpendTime) const
{
    vCoins.clear();
...
bool compareCOutput(COutput output1, COutput output2)
{
    const CWalletTx *pcoin1 = output1.tx;
    int index1 = output1.i;
    int64_t n1 = pcoin1->tx->vout[index1].nValue;
    const CWalletTx *pcoin2 = output2.tx;
    int index2 = output2.i;
    int64_t n2 = pcoin2->tx->vout[index2].nValue;
    return (n1 > n2);
}


// Compares two intervals according to staring times.
bool compareCWalletTx(std::pair<const CWalletTx*,unsigned int> p1, std::pair<const CWalletTx*,unsigned int> p2)
{
    int64_t n1 = p1.first->tx->vout[p1.second].nValue;
    int64_t n2 = p2.first->tx->vout[p2.second].nValue;
    return (n1 > n2);
}


bool CWallet:: SelectCoinsForStaking(unsigned int nSpendTime, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    LogPrint(BCLog::ALL, "SelectCoinsForStaking nSpendTime %d\n", nSpendTime);
    std::vector<COutput> vCoins;
    ...
    nValueRet = 0;
    if (!vCoins.empty()) {
        std::sort(vCoins.begin(), vCoins.end(), compareCOutput);
        for (const auto &output : vCoins) {
            ...
            if (n >= nTargetValue) {
                // If input value is greater or equal to target then simply insert it into the current subset and exit
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // curvehash: set time
...
//                }
                ...
                // curvehash: sub-cent change is moved to fee
...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    if (!SelectCoinsForStaking(GetTime(), nBalance - nReserveBalance, setCoins, nValueIn))
        return 0;
...
// curvehash: create coin stake transaction
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, CMutableTransaction& txNew)
{
    ...
    if (gArgs.IsArgSet("-reservebalance") && !ParseMoney(gArgs.GetArg("-reservebalance", ""), nReserveBalance))
        return error("CreateCoinStake : invalid reserve balance amount");
    if (nBalance <= nReserveBalance) {
        LogPrint(BCLog::ALERT, "not enough nBalance %d nReserveBalance %d\n", nBalance, nReserveBalance);
        return false;
    }
    std::set<std::pair<const CWalletTx*,unsigned int> > setCoins;
    std::vector<std::pair<const CWalletTx*,unsigned int> > sortedCoins;
    std::vector<const CWalletTx*> vwtxPrev;
    CAmount nValueIn = 0;
    // Select coins with suitable depth
    if (!SelectCoinsForStaking(txNew.nTime, nBalance - nReserveBalance, setCoins, nValueIn))
        return false;
    if (setCoins.empty()) {
    ...
    std::sort(sortedCoins.begin(), sortedCoins.end(), compareCWalletTx);
    CAmount nCredit = 0;
    CScript scriptPubKeyKernel;
    for (const auto &pcoin : sortedCoins) {
        CDiskTxPos postx;
        ...
        if (CheckKernel(nBits, pindexPrev, header, tx, prevoutStake, txNew.nTime)) {
            // Found a kernel
    ...
    for (const auto &pcoin : sortedCoins) {
        ...
        if (txNew.vout.size() == 2 && ((scriptPubKey == scriptPubKeyKernel || scriptPubKey == txNew.vout[1].scriptPubKey))
