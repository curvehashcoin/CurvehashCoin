...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        // curvehash: enforce minimum output amount
        if (txout.nValue < 0)
...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const Consensus::Params& params)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (!MoneyRange(coin.out.nValue) || !MoneyRange(nValueIn)) {chainActive.Tip();
    ...
    if (tx.IsCoinStake())
    {
        // curvehash: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
        if (!GetCoinAge(tx, inputs, chainActive.Tip(), nCoinAge, nSpendHeight))
    ...
    else
    {
        ...
        // curvehash: enforce transaction fees for every block
